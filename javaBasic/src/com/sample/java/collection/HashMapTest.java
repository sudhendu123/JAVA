...
public class HashMapTest {
	...
	public static void main(String[] args) {
		...
		removed.add("a");
		List<String> nullEl = hmapString.entrySet().stream().filter(q -> q.getValue() != null).map(q -> q.getKey())
		...
		List<String> emptyList = hmap.values().stream().filter(a -> a.equals("0")).map(q -> q)
				.collect(Collectors.toList());
		...
		Set<String> mapValues = hmap.values().stream().sorted().collect(Collectors.toSet());
		System.out.println("mapValues :" + mapValues);
		List<Integer> mapKeys = hmap.keySet().stream().sorted().collect(Collectors.toList());
		...
		List<Integer> idCondition = objMap.entrySet().stream().filter(m -> m.getValue().getId() != 2)
		...
		Map<Integer, List<String>> result = groupByMap.entrySet().stream().collect(
